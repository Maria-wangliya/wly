...
XZoom.prototype = {
	...
	_init:function(options){
		...
		smallImg.onload = ()=>{
			...
			if(opt.position === 'right'){
				...
				big.style.left = this.ele.offsetLeft - opt.oofsetWidth - opt.gap + 'px';
				big.style.top = this.ele.offsetTop + 'px';
		...
		this.ele.onmousemove = (e)=>{
			var left = e.clientX - this.ele.children[0].offsetLeft - this.zoom.offsetWidth/2;
			var top = e.clientY - this.ele.children[0].offsetTop - this.zoom.offsetHeight/2;
			// 限定left，top值
			if(left<0){
				left = 0;
	...
	show:function(){
		...
		if(bigImg.complete){
			...
			// 定义放大镜尺寸
			this.zoom.style.width = this.big.offsetWidth/this.ratio/3+ 'px';
			this.zoom.style.height = this.big.offsetHeight/this.ratio/3+ 'px';
		}
